        ...
        if 'error' in cat_status:
            ...
            for feature, available in cat_status.items():
                if isinstance(available, bool):
                    ...
                    print(f"  📋 {feature.replace('_', ' ').title()}: {len(available)} functions")
                else:
...
def get_available_features() -> Dict[str, List[str]]:
    ...
    # Core features
    features['core'] = [
        'analyze', 'get_data_info', 'analyze_numeric', 'analyze_categorical',
        'detect_outliers', 'validate_dataframe', 'summary_stats', 'box_plots',
        'create_interactive_plots'
    ]
    
    # Visualization features
    features['visualization'] = [
        'correlation_matrix', 'distribution_plots'
    ]
...
def create_utility_report() -> str:
    ...
    try:
        ...
        report.append(f"  Memory: {sys_info['memory_total_gb']:.1f} GB total, {sys_info['memory_available_gb']:.1f} GB available")
        
        if 'error' not in sys_info['disk_usage']:
            disk = sys_info['disk_usage']
            report.append(f"  Disk: {disk['total_gb']:.1f} GB total, {disk['free_gb']:.1f} GB free")
    ...
    try:
        ...
        for category, cat_status in status.items():
            if 'error' not in cat_status:
                available_count = sum(1 for v in cat_status.values() if isinstance(v, bool) and v)
                total_count = sum(1 for v in cat_status.values() if isinstance(v, bool))
                report.append(f"  {category.replace('_', ' ').title()}: {available_count}/{total_count} available")
            else:
                report.append(f"  {category.replace('_', ' ').title()}: ❌ {cat_status['error']}")
    except Exception as e:
