...
def correlation_matrix(df: pd.DataFrame, 
                     ...
                     output_dir: str = "./quickinsights_output") -> None:
    """
    ...
    if save_plot:
        ...
        plt.savefig(f"{output_dir}/correlation_matrix_{method}.png", 
                   dpi=300, bbox_inches='tight')
        print(f"💾 Korelasyon matrisi kaydedildi: {output_dir}/correlation_matrix_{method}.png")
...
def distribution_plots(df: pd.DataFrame, 
    ...
    if save_plots:
        output_dir = create_output_directory(output_dir)
        plt.savefig(f"{output_dir}/distribution_plots_all.png", 
                   dpi=300, bbox_inches='tight')
        print(f"💾 Tüm dağılım grafikleri kaydedildi: {output_dir}/distribution_plots_all.png")
...
def create_interactive_plots(df: pd.DataFrame, 
    ...
    if len(numeric_cols) >= 2:
        ...
        fig.update_layout(
            title='Korelasyon Matrisi (İnteraktif)',
            width=800,
            height=600
        )
    ...
    if len(numeric_cols) > 0:
        ...
        for idx, col in enumerate(numeric_cols):
            ...
            fig.add_trace(
                go.Histogram(x=col_data, name=col, nbinsx=30),
                row=row, col=col_idx
            )
...
def box_plots(df: pd.DataFrame,
    ...
    if save_plot:
        output_dir = create_output_directory(output_dir)
        plt.savefig(f"{output_dir}/box_plots_all.png", 
                   dpi=300, bbox_inches='tight')
