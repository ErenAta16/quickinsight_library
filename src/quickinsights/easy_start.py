...
def easy_load_data(
    file_path: Union[str, Path], encoding: str = "utf-8", auto_detect: bool = True
) -> pd.DataFrame:
    """
        ...
        if file_extension == ".csv":
            ...
            for sep in separators:
                for enc in encodings:
                    try:
                        df = pd.read_csv(
                            file_path, separator=sep, encoding=enc, low_memory=False
                        )
...
def easy_analyze(
    df: pd.DataFrame, target: Optional[str] = None, quick: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        # Run analysis
        quick_result = quick_insight(
            df, target_column=target, sample_size=sample_size, include_viz=False
        )
        ...
        simplified_result = {
            ...
            "data_quality": {
                ...
                "level": _get_simple_quality_level(
                    quality_result["missing_data"]["missing_percentage"]
                ),
        ...
        if target and "target_analysis" in quick_result:
            ...
            if quick_result["target_analysis"]["variable_type"] == "categorical":
                simplified_result["target_variable_analysis"][
                    "class_distribution"
                ] = quick_result["target_analysis"]["class_distribution"]
                simplified_result["target_variable_analysis"][
                    "balance_assessment"
                ] = quick_result["target_analysis"]["balance_assessment"]
...
def easy_clean(
    df: pd.DataFrame, target: Optional[str] = None, save_original: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for step in cleaning_result["cleaning_steps"]:
            ...
            if step_name == "missing_data_handling" and step["actions_taken"]:
                simple_result["performed_operations"].append(
                    f"✅ Missing data filled ({len(step['actions_taken'])} columns)"
                )
            elif step_name == "duplicate_handling" and step["removed_count"] > 0:
                simple_result["performed_operations"].append(
                    f"✅ {step['removed_count']} duplicate rows removed"
                )
            elif step_name == "outlier_handling" and step["actions_taken"]:
                simple_result["performed_operations"].append(
                    f"✅ Outliers handled ({len(step['actions_taken'])} columns)"
                )
            elif step_name == "data_type_optimization" and step["optimized_columns"]:
                simple_result["performed_operations"].append(
                    f"✅ Data types optimized ({len(step['optimized_columns'])} columns)"
                )
        ...
        if row_change > 0:
            simple_result[
                "summary"
            ] = f"Data cleaning complete! {row_change:,} rows and {original_memory - new_memory:.1f} MB memory saved."
        else:
            simple_result[
                "summary"
            ] = f"Data cleaning complete! {original_memory - new_memory:.1f} MB memory saved."
...
def easy_visualize(
    df: pd.DataFrame, target: Optional[str] = None, max_plots: int = 5
) -> Dict[str, Any]:
    """
    ...
    recommendations = {"plot_recommendations": [], "code_examples": [], "tips": []}

    # Basic distribution plots
    if len(numeric_cols) > 0:
        recommendations["plot_recommendations"].append(
            "📈 Distribution plots for numerical variables"
        )
        recommendations["code_examples"].append(
            f"df[{numeric_cols[:3]}].hist(figsize=(12, 8))"
        )
    ...
    if target and target in df.columns:
        if target in df.select_dtypes(include=[np.number]).columns:
            recommendations["plot_recommendations"].append(
                f"🎯 Distribution of {target} target variable"
            )
            recommendations["code_examples"].append(f"df['{target}'].hist(bins=30)")

            if len(numeric_cols) > 0:
                recommendations["plot_recommendations"].append(
                    f"📊 Scatter plots of {target} with other variables"
                )
                for col in numeric_cols[:2]:
                    recommendations["code_examples"].append(
                        f"df.plot.scatter(x='{col}', y='{target}')"
                    )
        else:
            recommendations["plot_recommendations"].append(
                f"🎯 Class distribution of {target}"
            )
            recommendations["code_examples"].append(
                f"df['{target}'].value_counts().plot(kind='pie')"
            )
    ...
    if missing_count > 0:
        ...
        recommendations["code_examples"].append(
            "import seaborn as sns; sns.heatmap(df.isnull(), cbar=True)"
        )
    ...
    if max_plots > 0 and len(recommendations["plot_recommendations"]) > max_plots:
        recommendations["plot_recommendations"] = recommendations[
            "plot_recommendations"
        ][:max_plots]
        recommendations["code_examples"] = recommendations["code_examples"][:max_plots]

    print(
        f"✅ {len(recommendations['plot_recommendations'])} visualization recommendations prepared!"
    )
...
def easy_export(
    df: pd.DataFrame, filename: str = "cleaned_data", format: str = "csv"
) -> str:
    """
