...
def quick_insight(
    ...
    if sample_size and len(df) > sample_size:
        ...
        print(
            f"🔸 Large dataset detected. Using sample of {sample_size:,} rows for quick analysis."
        )
    else:
        df_sample = df.copy()
    ...
    # Görselleştirme
    if include_viz:
        result["visualizations"] = _create_quick_visualizations(
...
def _get_dataset_overview(
    df_full: pd.DataFrame, df_sample: pd.DataFrame
) -> Dict[str, Any]:
...
def _assess_data_quality(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if col not in numeric_cols:
            ...
            if unique_ratio > 0.95:  # Probably should be an ID column
                inconsistent_types.append(
                    f"{col}: too many unique values ({unique_ratio:.2%})"
                )
    ...
    return {
        ...
        "duplicates": {"count": duplicates, "percentage": duplicate_pct},
        "type_issues": inconsistent_types,
        "recommendations": _get_quality_recommendations(
            quality_score, missing_pct, duplicate_pct
        ),
    }
...
def _get_quality_recommendations(
    score: float, missing_pct: pd.Series, duplicate_pct: float
) -> List[str]:
    ...
    if missing_pct.max() > 50:
        ...
        recommendations.append(
            f"🔴 Column '{worst_col}' has {missing_pct.max():.1f}% missing - consider dropping"
        )
    elif missing_pct.max() > 20:
        recommendations.append(
            "🟡 High missing values detected - investigate imputation strategies"
        )

    if duplicate_pct > 10:
        recommendations.append(
            f"🔴 {duplicate_pct:.1f}% duplicate rows found - clean before analysis"
        )
        ...
        if len(numeric_cols) > 1:
            ...
            if high_corr:
                for col1, col2, corr_val in high_corr[:3]:  # Top 3
                    insights.append(
                        f"🔗 Strong correlation detected: {col1} ↔ {col2} (r={corr_val:.2f})"
                    )
    ...
    if len(categorical_cols) > 0:
        for col in categorical_cols[:3]:  # Analyze top 3 categorical columns
            ...
            if unique_count == total_count:
                ...
                insights.append(
                    f"📝 '{col}' has very high cardinality ({unique_count} unique values)"
                )
                ...
                insights.append(
                    f"📈 '{col}': '{top_value}' dominates with {top_pct:.1%} of values"
                )

    # Dataset size insights
    if len(df) < 100:
        insights.append(
            "⚠️ Small dataset detected - statistical insights may be limited"
        )
...
def _generate_recommendations(
    ...
    if target_column and target_column in df.columns:
        if target_column in numeric_cols:
            recommendations.append(
                "📊 Regression analysis recommended for numeric target"
            )
            recommendations.append(
            ...
            if unique_targets == 2:
                ...
                recommendations.append(
                    f"🎯 Multi-class classification with {unique_targets} classes"
                )
    ...
    if len(categorical_cols) > 0:
        ...
        if high_card_cols:
            recommendations.append(
                f"🏷️ High cardinality categorical features detected: {high_card_cols[:2]} - consider encoding strategies"
            )

    # Analysis workflow recommendations
    recommendations.append(
        "📈 Next steps: Detailed EDA → Feature Engineering → Model Selection"
    )
...
def _analyze_target_variable(df: pd.DataFrame, target_column: str) -> Dict[str, Any]:
    ...
    analysis = {
        ...
        "missing_percentage": (
            target_series.isnull().sum() / len(target_series) * 100
        ).round(2),
    }
    ...
    if pd.api.types.is_numeric_dtype(target_series):
        ...
        if abs(analysis["statistics"]["skewness"]) > 1:
            analysis[
                "distribution_note"
            ] = f"Highly skewed distribution (skew={analysis['statistics']['skewness']:.2f})"
        elif abs(analysis["statistics"]["skewness"]) > 0.5:
            analysis[
                "distribution_note"
            ] = f"Moderately skewed distribution (skew={analysis['statistics']['skewness']:.2f})"
        else:
        ...
        analysis["class_balance"] = {
            ...
            "most_frequent_pct": (
                value_counts.iloc[0] / len(target_series) * 100
            ).round(2),
            "least_frequent": value_counts.index[-1],
            "least_frequent_pct": (
                value_counts.iloc[-1] / len(target_series) * 100
            ).round(2),
        }
        ...
        if max_pct > 0.9:
            analysis[
                "balance_assessment"
            ] = "Severely imbalanced - consider resampling techniques"
        elif max_pct > 0.7:
            analysis[
                "balance_assessment"
            ] = "Moderately imbalanced - monitor model performance"
        else:
