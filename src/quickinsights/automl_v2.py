...
def intelligent_model_selection(
    ...
    def auto_hyperparameter_tuning(
        ...
        final_model = self._train_final_model(model, best_params, X, y)
        ...
        for name, model in models:
            try:
                if task_type == "classification":
                    ...
                    cv_scores = cross_val_score(
                        model, X, y, cv=cv_folds, scoring="r2", n_jobs=n_jobs
                    )
...
class AutoMLPipeline:
            ...
            if X.shape[1] > 100:
                ...
                if self.task_type == "classification":
                    ...
                    selector = SelectKBest(
                        score_func=f_regression, k=min(50, X.shape[1])
                    )
                ...
                print(
                    f"✅ Feature selection completed. Selected {X_selected.shape[1]} features"
                )
                return X_selected
            else:
        ...
        try:
            ...
            if evaluation_results:
                best_name = max(
                    ...
                    if "error" not in evaluation_results[x]
                    else 0,
                )
    ...
    def predict_proba(self, X):
        ...
        if self.task_type != "classification":
            raise ValueError(
                "Probability predictions only available for classification tasks"
            )
