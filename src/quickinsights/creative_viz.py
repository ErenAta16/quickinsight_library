...
class CreativeVizEngine:
    ...
    def create_gantt_chart(
        self, start_col: str, end_col: str, task_col: str
    ) -> go.Figure:
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    x=[
                        (self.df[start_col].iloc[i], self.df[end_col].iloc[i])
    ...
    def create_sankey_diagram(
        self, source_cols: List[str], target_col: str
    ) -> go.Figure:
    ...
    def create_box_plot(self, x_col: str, y_col: str) -> go.Figure:
        ...
        for category in self.df[x_col].unique():
            fig.add_trace(
                go.Box(y=self.df[self.df[x_col] == category][y_col], name=category)
            )
    ...
    def create_density_plot(self, x_col: str, color_col: str = None) -> go.Figure:
        ...
        if color_col:
            ...
            fig = px.histogram(
                self.df, x=x_col, histnorm="probability density", title="Density Plot"
            )
    ...
    def create_quiver_plot(
        self, x_col: str, y_col: str, u_col: str, v_col: str
    ) -> go.Figure:
        ...
        for i in range(0, len(self.df), 5):  # Her 5. noktada vektör
            fig.add_trace(
                go.Scatter(
                    x=[
                        self.df[x_col].iloc[i],
                        self.df[x_col].iloc[i] + self.df[u_col].iloc[i],
                    ],
                    y=[
                        self.df[y_col].iloc[i],
                        self.df[y_col].iloc[i] + self.df[v_col].iloc[i],
                    ],
                    mode="lines",
    ...
    def create_cone_plot(
        self, x_col: str, y_col: str, z_col: str, u_col: str
    ) -> go.Figure:
        """Cone plot oluşturur"""
        # Basit 3D scatter plot olarak göster
        fig = go.Figure(
            data=[
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        size=self.df[u_col], color=self.df[u_col], colorscale="Viridis"
                    ),
                )
            ]
        )
    ...
    def create_volume_plot(
        self, x_col: str, y_col: str, z_col: str, value_col: str
    ) -> go.Figure:
    ...
    def create_scatter_mapbox(
        self, lat_col: str, lon_col: str, color_col: str
    ) -> go.Figure:
    ...
    def create_choropleth_mapbox_with_hover(
        self, location_col: str, color_col: str, hover_col: str
    ) -> go.Figure:
    ...
    def create_scatter_mapbox_with_hover(
        self, lat_col: str, lon_col: str, color_col: str, size_col: str
    ) -> go.Figure:
    ...
    def create_density_mapbox_with_hover(
        self, lat_col: str, lon_col: str, value_col: str
    ) -> go.Figure:
    ...
    def create_line_mapbox_with_hover(
        self, lat_col: str, lon_col: str, color_col: str, value_col: str
    ) -> go.Figure:
    ...
    def create_polygon_mapbox_with_hover(
        self, lat_col: str, lon_col: str, value_col: str
    ) -> go.Figure:
